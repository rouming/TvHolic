TEMPLATE = lib
!win32:VERSION = 2.0.0
DESTDIR = $${BRISA_BUILD_TREE}/lib
win32:DLLDESTDIR = $${BRISA_BUILD_TREE}/bin
target.path = $${BRISA_INSTALL_LIBS}
win32:dlltarget.path = $${BRISA_INSTALL_BINS}
headers.path = $${BRISA_INSTALL_HEADERS}/$${CLEAN_TARGET}
contains(CONVENIENCE, $${CLEAN_TARGET}){
    for(header,HEADERS){
        headers.files += $$header
    }
    headers.files += $${BRISA_SOURCE_TREE}/include/$${CLEAN_TARGET}/*
}
...
macx {
    ...
    QMAKE_LFLAGS += -F$${BRISA_BUILD_TREE}/lib
}
